...
#define QQAUTHOR			@"xgadget"

@implementation QQTPostView
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) 
	{
		...
		CGRect rect = CGRectMake(frame.origin.x+frame.size.width-180, btnStartY+9, 174, 32);
		...
		[m_PostButton setBackgroundImage:[[UIImage imageNamed:@"qqpost.png"] stretchableImageWithLeftCapWidth:10.0 topCapHeight:0.0] forState:UIControlStateNormal];
		[m_PostButton setBackgroundImage:[[UIImage imageNamed:@"qqpost2.png"] stretchableImageWithLeftCapWidth:10.0 topCapHeight:0.0] forState:UIControlStateHighlighted];
		[m_PostButton setBackgroundImage:[[UIImage imageNamed:@"qqpost1.png"] stretchableImageWithLeftCapWidth:10.0 topCapHeight:0.0] forState:UIControlStateDisabled];
...
- (void) connectionDidFinishLoading:(NSURLConnection *)connection 
{
	if(connection != nil)
		[connection release];
    [CustomModalAlertView SimpleSay:@"QQ腾讯微博发布成功！" closeButton:[StringFactory GetString_Close]];
	[self DismissMe];
}
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error 
{
	if(connection != nil)
		[connection release];
    [CustomModalAlertView SimpleSay:@"QQ腾讯微博发布失败！" closeButton:[StringFactory GetString_Close]];
	[self DismissMe];
}
...
- (void)sendNewPost
{
	...
	NSURLConnection* connection	= [[api publishMsgWithConsumerKey:[QOauthConfigure GetAppKey] 
									  consumerSecret:[QOauthConfigure GetAppSecret] 
									  accessTokenKey:[QOauthConfigure GetTokenKey] 
								   accessTokenSecret:[QOauthConfigure GetTokenSecret] 
											 content:m_Draft.text 
										   imageFile:(self.m_GameIcon != nil && ![self.m_GameIcon isEqualToString:@""]) ? self.m_GameIcon : nil 
...
- (void)PostButtonClick
{
	...
	{
		//[ModalAlert say:@"还未登录QQ微博帐户,不能发布微博!"];
...
- (void)FollowMe
{
	...
	NSString *retString = [api follow:[QOauthConfigure GetAppKey] 
						consumerSecret:[QOauthConfigure GetAppSecret] 
					   accessTokenKey:[QOauthConfigure GetTokenKey] 
					accessTokenSecret:[QOauthConfigure GetTokenSecret] 
							 username:QQAUTHOR 
...
- (void)SetStealthMode
{
	...
	m_CloseButton.hidden = NO;
	m_Spinner.hidden = YES;
    
    self.backgroundColor = [UIColor clearColor];
}

- (void)StealthPostQQTweet:(NSString*)gameTitle withMessage:(NSString*)gameMsg withImage:(NSString*)gameIcon withURL:(NSString*)gameURL
{
	self.m_GameTitle = gameTitle;//[NSString stringWithFormat:@"%@", gameTitle];
	self.m_GameMsg = gameMsg;//[NSString stringWithFormat:@"%@", gameMsg];
	self.m_GameIcon = gameIcon;//[NSString stringWithFormat:@"%@", gameIcon];
