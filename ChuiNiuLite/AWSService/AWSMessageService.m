...
@implementation AWSMessageService
...
-(void)postFailureProcess
{
    [GUIEventLoop SendEvent:GUIID_AWSSERVICE_FAILED_EVENT eventSender:nil];
}


//-(id)init
-(id)init:(NSString*)accessKey withSecret:(NSString*)secretKey withTopic:(NSString*)topicName withQueue:(NSString*)queueName
{
    self = [super init];
    if (self != nil) 
    {
        ...
        snsClient = [[AmazonSNSClient alloc] initWithAccessKey:accessKey withSecretKey:secretKey];
        sqsClient = [[AmazonSQSClient alloc] initWithAccessKey:accessKey withSecretKey:secretKey];
...
-(NSString *)createTopic
{
    @try 
    {
        SNSCreateTopicRequest  *ctr = [[[SNSCreateTopicRequest alloc] initWithName:_topicName] autorelease];
        ...
        SNSSetTopicAttributesRequest *st = [[[SNSSetTopicAttributesRequest alloc] initWithTopicArn:response.topicArn andAttributeName:@"DisplayName" andAttributeValue:_topicName] autorelease];
...
-(NSString *)createQueue
{
    @try 
    {
        SQSCreateQueueRequest  *cqr      = [[[SQSCreateQueueRequest alloc] initWithQueueName:_queueName] autorelease];
...
-(NSMutableArray *)getMessagesFromQueue
{
    @try 
    {
        ...
        rmr.visibilityTimeout   = [NSNumber numberWithInt:5];
...
-(void)addMessageRetentionToQueueForTopic:(NSString *)theQueueUrl retention:(int)holdTime
{
    @try 
    {
        NSMutableDictionary *attributes = [[[NSMutableDictionary alloc] init] autorelease];
        NSNumber* objTime = [[[NSNumber alloc] initWithInt:holdTime] autorelease];
...
-(NSString *)findTopicArn
{
    @try 
    {
        NSString *topicNameToFind = [NSString stringWithFormat:@":%@", _topicName];
...
-(NSString *)findQueueUrl
{
    @try 
    {
        NSString              *queueNameToFind = [NSString stringWithFormat:@"/%@", _queueName];
...
-(void)setQueueMessageRetentionTime:(int)nTimesSecond
{
    [self addMessageRetentionToQueueForTopic:queueUrl retention:nTimesSecond];
}
