...
@interface NOMSpotGasStationView ()
{
    NOMSpotViewButtonPanel*                             m_ButtonPanel;
    NOMSpotGasStationViewFrame*                         m_SpotView;
...
@implementation NOMSpotGasStationView
...
-(void)InitializeSubViews
{
    ...
    m_SpotView = [[NOMSpotGasStationViewFrame alloc] initWithFrame:rect];
...
{
    [m_SpotView SetCarWashType:nType];
}
...
{
    [m_SpotView SetPrice:dPrice];
}

-(void)SetPriceUnit:(int16_t)nUnit
{
    [m_SpotView SetPriceUnit:nUnit];
}

-(int16_t)GetCarWashType
{
    return [m_SpotView GetCarWashType];
}
...
-(double)GetPrice
{
    return [m_SpotView GetPrice];
}

-(int16_t)GetPriceUnit
{
    return [m_SpotView GetPriceUnit];
}
