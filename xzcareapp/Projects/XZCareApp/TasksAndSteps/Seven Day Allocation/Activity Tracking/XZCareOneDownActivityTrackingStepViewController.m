...
#import "XZCareOneDownActivityTrackingStepViewController.h"
...
@interface XZCareOneDownActivityTrackingStepViewController () <XZCarePieGraphViewDatasource>
- (IBAction)resetTaskStartDate:(id)sender;

@property (weak, nonatomic) IBOutlet UILabel *daysRemaining;
@property (weak, nonatomic) IBOutlet XZCarePieGraphView *chartView;
...
@implementation XZCareOneDownActivityTrackingStepViewController
...
- (NSDate *)checkSevenDayFitnessStartDate
{
    ...
    if (!fitnessStartDate) {
        ...
        XZCareCoreAppDelegate *appDelegate = (XZCareCoreAppDelegate *)[[UIApplication sharedApplication] delegate];
        appDelegate.sevenDayFitnessAllocationData = [[XZCareFitnessAllocation alloc] initWithAllocationStartDate:fitnessStartDate];
...
- (UIColor *)pieGraphView:(XZCarePieGraphView *)pieGraphView colorForSegmentAtIndex:(NSInteger)index
{
    return [[self.allocationDataset valueForKey:kDatasetSegmentColorKey] objectAtIndex:index];
}

- (NSString *)pieGraphView:(XZCarePieGraphView *)pieGraphView titleForSegmentAtIndex:(NSInteger)index
{
    return [[self.allocationDataset valueForKey:kDatasetSegmentKey] objectAtIndex:index];
}

- (CGFloat)pieGraphView:(XZCarePieGraphView *)pieGraphView valueForSegmentAtIndex:(NSInteger)index
{
