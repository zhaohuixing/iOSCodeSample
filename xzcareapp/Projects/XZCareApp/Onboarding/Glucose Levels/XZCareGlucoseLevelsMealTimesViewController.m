//
//  TCMCareGlucoseLevelsMealTimesViewController.m
//  TACMCareDiabeteCare
//
//
...
@interface XZCareGlucoseLevelsMealTimesViewController ()
...
@implementation XZCareGlucoseLevelsMealTimesViewController
...
- (void)setupSchedules
{
    ...
    for (NSDictionary *checkTime in self.glucoseCheckTimes)
    {
        ...
        for (NSNumber *hour in scheduleTimes)
        {
            ...
            {
                XZCareLogDebug(@"Duplicate hour: %@", hour);
    ...
    XZCareCoreDBSchedule *glucoseSchedule = nil;
    ...
    if ([entries count] == 0)
    {
        glucoseSchedule = [XZCareCoreDBSchedule newObjectForContext:appDelegate.dataSubstrate.mainContext];
        
        XZCareLogDebug(@"Sleep/Wake: %@/%@", userSleepTime, userWakeTime);
        XZCareLogDebug(@"Glucose schedule: %@", scheduleString);
        ...
        if (!saveSuccess)
        {
            XZCareLogError2(glucoseScheduleError);
        }
        
        // Send the schedule notification
        [[NSNotificationCenter defaultCenter] postNotificationName:XZCareScheduleUpdatedNotification
                                                            object:nil];
    }
    else
    {
...
//    localContext.parentContext = appDelegate.dataSubstrate.persistentContext;
    ...
    XZCareCoreDBSchedule *glucoseSchedule = (XZCareCoreDBSchedule *)[localContext objectWithID:schedule.objectID];
    XZCareCoreDBTask *glucoseLogTask = [XZCareCoreDBTask taskWithTaskID:kGlucoseLogSurveyIdentifier
                                            inContext:localContext];
    
    NSArray *listOfScheduledTasks = [schedule.scheduleTasks allObjects];
    ...
    for (NSDate *taskStartDate in [entryDates allObjects]) {
        ...
        for (NSNumber *hour in newlyAddedHours) {
            ...
            XZCareCoreDBScheduledTask *scheduledTaskForNewEntry = [XZCareCoreDBScheduledTask newObjectForContext:appDelegate.dataSubstrate.mainContext];
...
- (void)saveGlucoseSetup:(NSArray *)scheduleHours
{
    ...
    XZCareLogDebug(@"Glucose meal times: %@", self.glucoseCheckTimes);
