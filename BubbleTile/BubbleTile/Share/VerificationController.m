...
- (void)registerDelegate:(id<VerificationControllerDelegate>)delegate
{
    _delegate = delegate;
}
...
- (BOOL)doesTransactionInfoMatchReceipt:(NSString*) receiptString
{
    ...
    {
        if(_delegate)
            [_delegate setCurrentVerificationProductID:[purchaseInfoFromTransaction objectForKey:@"product-id"]];
    }
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data
{
    ...
    if (isOk)
    {
        ...
        if(_delegate)
            [_delegate currentProductPurchaseVerificationDone];
    }
}
...
{
    NSData* data = [NSData dataWithBytesNoCopy:(void*)input length:length];
    NSString* string = [[NSString alloc] initWithData:data encoding:NSASCIIStringEncoding];
    return [VerificationController encodeString:string];
}
...
{
    NSString* pRet = [VerificationController decodeString:input];
    if(length)
        *length = pRet.length;
    return pRet;
}
...
{
    //NSString* string = [VerificationController encode:(const uint8_t *)(buf) length:size];
    //char* pRet = (char*)[string UTF8String];
    //return pRet;

    NSData* data = [NSData dataWithBytesNoCopy:(void*)buf length:size];
    NSString* string = [[NSString alloc] initWithData:data encoding:NSASCIIStringEncoding];
    char* pRet = (char*)[[VerificationController encodeString:string] UTF8String];
    return pRet;
}
...
{
    NSString* result = [VerificationController decodeString:[NSString stringWithCString:s encoding:NSASCIIStringEncoding]];
    *data_len = result.length;
    return (void*)[result UTF8String];
}
