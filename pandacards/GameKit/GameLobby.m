...
-(BOOL)IsLive
{
    BOOL bRet = (self.m_RealGameLobby != nil || m_GKBTeSession != nil);
    return bRet;
}
...
-(void)Shutdown
{
    if(m_RealGameLobby != nil)
    {
        ...
        [m_RealGameLobby disconnect];
        [m_RealGameLobby release];
        m_RealGameLobby = nil;
    }
    else if(m_GKBTeSession != nil)
    {
...
-(void)HandleLobbyInvitation
{
    ...
    {    
        ...
        [GKMatchmaker sharedMatchmaker].inviteHandler = ^(GKInvite *acceptedInvite, NSArray *playersToInvite) 
        {
            ...
            BOOL bCanAcceptInvitation = [m_GamePostDelegate CanAcceptInvitation];
            if(bCanAcceptInvitation == NO)
                return;
...
-(NSString*)GetPlayerID:(int)index
{
    ...
    else if(m_GKBTeSession != nil)
    {
        if(index == 0)
            ...
            szRet = m_GKBTPeerID; //m_GKBTeSession.peerID;
    }
...
-(BOOL)HasPlayer:(NSString*)playerID
{
    ...
    {
        //if(m_GKBTeSession.peerID)
        if(m_GKBTPeerID)
        {
            //if([playerID isEqualToString:m_GKBTeSession.peerID])
            if([playerID isEqualToString:m_GKBTPeerID])
            {
                bRet = YES;
                return bRet;
            }
        ...
        if(m_MyGKBTID)
        {
            if([playerID isEqualToString:m_MyGKBTID])
            {
                bRet = YES;
                return bRet;
            }
...
-(BOOL)SendMessageToAllplayers:(GameMessage*)msg
{
    ...
    else if(m_GKBTeSession != nil)
    {
        ...
        {    
            ...
            {
                NSError* myerror = nil;
                bRet = [m_GKBTeSession sendDataToAllPeers:data withDataMode:GKSendDataUnreliable error:&myerror];
                if(myerror == nil)
                {
                    bRet = YES;
                }
...
-(BOOL)SendMessage:(GameMessage*)msg toPlayer:(NSString*)playerID
{
    ...
    if(self.m_RealGameLobby != nil && [self HasPlayer:playerID])
    {
        if(msg != nil )
        {    
            NSData* data = [msg.m_GameMessage dataUsingEncoding:NSUTF8StringEncoding];
    ...
    else if(m_GKBTeSession)
    {
        ...
        {    
            ...
            if(data != nil)
            {
                NSError* myerror = nil;
                NSArray* array = [NSArray arrayWithObject: playerID];
                bRet = [m_GKBTeSession sendData:data toPeers:array withDataMode:GKSendDataUnreliable error:&myerror];
                if(myerror == nil)
                {
                    bRet = YES;
                }
...
-(void)CreateGKBTSession:(id<GKSessionDelegate>)delegate
{
    ...
    { 
        ...
        m_GKBTeSession = [[GKSession alloc] initWithSessionID:@"Panda Cards GK Bluetooth session" displayName:m_MyGKBTName sessionMode:GKSessionModePeer]; 
        m_GKBTeSession.delegate = delegate; 
    } 
}
...
-(void)ResetGKBluetoothSession
{
    if(m_GKBTeSession)
    {
        [m_GKBTeSession disconnectFromAllPeers]; 
            
        m_GKBTeSession.available = NO; 
            
        [m_GKBTeSession setDataReceiveHandler: nil withContext: NULL]; 
            
        m_GKBTeSession.delegate = nil; 
        [m_GKBTeSession release];
        m_GKBTeSession = nil;
        if(m_GKBTPeerID)
        {
            [m_GKBTPeerID release];
            m_GKBTPeerID = nil;
        }
    }
}
...
{
    m_MyGKBTID = myID;
    m_MyGKBTName = [m_GKBTeSession displayNameForPeer:m_MyGKBTID];
}

-(NSString*)GetGKBTPeerName
{
    NSString* szRet = @"Unknown";
    if(m_GKBTeSession && m_GKBTPeerID)
    {
        szRet = [m_GKBTeSession displayNameForPeer:m_GKBTPeerID];
    }
    return szRet;
}
