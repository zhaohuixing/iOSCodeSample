...
-(void)InitiPhoneAddViews:(BOOL)bEnable
{
    ...
    {    
        ...
        m_AdmobView.adUnitID = [ApplicationConfigure GetAdMobPublishKey];
...
-(void)InitiPadAddViews
{
    ...
    {    
        ...
        m_AdmobView728.adUnitID = [ApplicationConfigure GetAdMobPublishKey];
...
/*    if([ApplicationConfigure iPADDevice])
    {    
        ...
        if(m_AdmobView == bannerView)
        {
            CGRect rect = CGRectMake((self.frame.size.width-468)/2.0, [GUILayout GetAdBannerHeight]-60, 468.0f, 60.0f);
            [m_AdmobView setFrame:rect];
            [self bringSubviewToFront:m_AdmobView];
        }
    }
