...
static CGPatternRef        m_GrayTexUIPattern;
...
+(void)InitializeResource
{
	...
	image = [ImageLoader LoadImageWithName:@"graytex.png"];
	width = CGImageGetWidth(image);
	height = CGImageGetHeight(image);
	m_GrayTexUIPattern = CGPatternCreate(image, CGRectMake(0, 0, width, height), transform, width, height,kCGPatternTilingNoDistortion, true, &callbacks);
...
+(void)ReleaseResource
{
    ...
    CGPatternRelease(m_GrayTexUIPattern);
}
...
+(void)DrawGrayTextureRect:(CGContextRef)context at:(CGRect)rect
{
    GLfloat fAlpha = 1.0;
    CGContextSetFillColorSpace(context, m_AlertUIPatternSpace);
    CGContextSetFillPattern(context, m_GrayTexUIPattern, &fAlpha);
	CGContextFillRect (context, rect);	
}
